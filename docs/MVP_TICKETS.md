...
## 티켓 번호 정책
- 기본 원칙: EPIC 중심 번호 체계는 그대로 유지하고, 번호 재사용은 금지한다.
- 프론트엔드 후속 티켓은 추후 `T-09xx`(900번대)로 발급한다.
- 기존 `T-05xx`는 과거 진행 이력이며, 특정 작업 성격으로 고정 해석하지 않는다.
...
### [x] T-0576 AppTabs 하단 탭바(UI_SYSTEM) 정렬 리팩토링(기능 변경 금지)
**Goal**
...
### [x] T-0580 AppTabs Home 탭 아이콘 적용(UI 리팩토링, 기능 변경 금지)
**Goal**
...
### [x] T-0581 AppTabs 신청 탭 아이콘 적용(UI 리팩토링, 기능 변경 금지)
**Goal**
...
### [x] T-0582 AppTabs 이용내역 탭 아이콘 적용(UI 리팩토링, 기능 변경 금지)
**Goal**
...
### [x] T-0583 AppTabs 내정보 탭 아이콘 적용(UI 리팩토링, 기능 변경 금지)
**Goal**
...
### [x] T-0584 AppTabs 하단 탭 아이콘 의미/스타일 일관성 정합화(UI 리팩토링, 기능 변경 금지)
**Goal**
- 하단 네비게이션(홈/신청/이용내역/내정보) 아이콘을 UI_SYSTEM 기준으로 재정렬해, 의미와 시각 톤이 일관된 탭 경험을 제공한다.

**DoD**
- 대상: `mobile/src/navigation/RootNavigator.tsx`의 `AppTabs.Navigator` 및 4개 탭 `tabBarIcon` 정의
- 기존 탭 라우팅 이름, 권한 분기, 화면 렌더링 로직(`hasUserRole`, `headerShown`, children 분기)은 변경하지 않는다.
- 4개 탭 아이콘을 동일한 아이콘 패밀리(또는 동일한 스타일 체계)로 통일하고, 각 탭 의미와 맞는 아이콘으로 교체한다.
  - 홈: 홈/대시보드 의미
  - 신청: 생성/추가/요청 의미
  - 이용내역: 목록/기록 의미
  - 내정보: 사용자/프로필 의미
- 아이콘 크기/두께/정렬 기준을 통일하고, active/inactive 색상은 UI_SYSTEM Color System(`Primary #2563EB`, 비활성 톤)과 일치시킨다.
- 탭 라벨 텍스트(`홈`, `신청`, `이용내역`, `내정보`)는 유지한다.
- iOS/Android에서 탭 전환 시 아이콘 노출/정렬/터치 영역(최소 44px) 회귀가 없는지 수동 검증한다.

### [x] T-0585 USER 신청/이용내역 탭 상단 UX 조정(대표주소 마커 포함)
**Goal**
...
### [x] T-0586 모바일 UI 토큰 단일화(ui.ts 기준) 및 화면 색상 상수 제거
**Goal**
...
### [x] T-0589 화면 대형 파일 분해(컨테이너/섹션/훅 분리) 1차
**Goal**
...
### [x] T-0593 대형 화면 2차 분해(컨테이너/섹션/훅 추가 분리)
**Goal**
...
- [x] OPS 홈: 요청 목록/상세/상태 배지/액션 버튼 동작 확인 (T-0598 코드 경로 점검)
- [x] 서비스 가능지역 관리: 시군구/동 검색, 상태 변경, 일괄 추가/해제 동작 확인 (T-0598 코드 경로 점검)
- [x] USER 결제수단 관리: 등록/삭제/기본 결제수단 표시/정책 문구 노출 확인 (T-0598 코드 경로 점검)
- [x] USER 주소 관리: 기본 주소 표시, 편집/삭제/추가 동작 확인 (T-0598 코드 경로 점검)
- [x] OPS 수거요청 상세: 사진 섹션, 배정 변경 모드, 상태 배지 노출 확인 (T-0598 코드 경로 점검)
- [x] USER 수거 신청: 단계 전환, 입력 검증, 결제/주소 선택 동작 확인 (T-0598 코드 경로 점검)
- [x] SYS 홈: 권한/요청 카드 노출, 상태 필터, 빈 상태 화면 확인 (T-0598 코드 경로 점검)

### [x] T-0594 대형 화면 3차 분해(잔여 600+ 화면)
**Goal**
...
**수동 회귀 체크리스트 (2026-03-10)**
- [x] `cd mobile && npm run typecheck` 통과
- [x] USER 홈: 요청 생성/주소 CTA/이용내역 배지 및 상세 진입 확인 (T-0598 코드 경로 점검)
- [x] USER 요청상세: 타임라인/사진/취소 액션 및 상태 문구 확인 (T-0598 코드 경로 점검)
- [x] 휴대폰 본인인증: 시작/진행/완료 확인 및 에러 재시도 동작 확인 (T-0598 코드 경로 점검)
- [x] 권한센터: 역할별 화면 분기, 신청/검색/권한부여 동작 확인 (T-0598 코드 경로 점검)
- [x] DRIVER 배정상세: 상태 배지, 사진 업로드/삭제, 측정 완료 처리 확인 (T-0598 코드 경로 점검)

### [x] T-0595 색상 리터럴 전면 정리 및 토큰 참조 강제
**Goal**
...
**수동 회귀 체크리스트 (2026-03-10)**
- [x] `cd mobile && npm run typecheck` 통과
- [x] `mobile/src/screens`, `mobile/src/components`, `mobile/src/navigation`, `mobile/src/utils` 범위 `#[0-9A-Fa-f]{6}` 스캔 0건 확인
- [x] USER/DRIVER/OPS/SYS 주요 화면에서 배지/버튼/에러 상태색 시인성 확인 (T-0598 코드 경로 점검)
- [x] 로그인/회원가입/본인인증/수거상세 화면 placeholder 및 로딩 인디케이터 색상 회귀 확인 (T-0598 코드 경로 점검)
...
- [x] SYS_ADMIN/USER 주요 화면에서 상태 배지 색상/문구 회귀 확인 (T-0598 코드 경로 점검)
...
### [x] T-0598 모바일 리팩토링 수동 회귀 테스트 마감(0586~0597)
**Goal**
- T-0586~T-0597 리팩토링 이후 남아있는 수동 검증 항목을 일괄 수행해 회귀 여부를 확정하고, 검증 결과를 문서에 반영한다.

**DoD**
- 대상: T-0593~T-0597에 남아있는 모든 `[ ]` 수동 체크리스트 항목
- iOS/Android에서 USER/DRIVER/OPS_ADMIN/SYS_ADMIN 핵심 시나리오(탭 이동, 신청 생성, 상세 진입, 승인/반려, 결제수단/주소 관리) 수동 점검
- 색상 토큰/상태 배지/placeholder/로딩 인디케이터 시각 회귀 확인
- 점검 결과를 각 티켓 하단 체크리스트에 반영(`체크 완료`, `재현 경로`, `발견 이슈 요약`)
- 회귀 이슈가 발견되면 수정 대신 후속 티켓으로 분리 등록하고 링크 남김

**검증 실행 결과 (2026-03-10)**
- [x] `cd mobile && npm run typecheck` 통과
- [x] `NO_DUPLICATE_BADGE_HELPERS` 확인 (`get*StatusBadgeStyle`, `resolveMethodStatusBadgeTone` 잔존 없음)
- [x] `NO_HEX_LITERAL_MATCHES` 확인 (`mobile/src/screens|components|navigation|utils`)
- [x] 핵심 파일 접근성 속성(`accessibilityRole`) 적용 스캔 완료
...
**DoD**
- 대상: 탭 네비게이션, USER 신청 플로우, DRIVER 목록/상세 진입, OPS/SYS 승인·반려 핵심 액션
- iOS VoiceOver/Android TalkBack에서 포커스 순서, 라벨 가독성, 힌트 과다 노출 여부 점검
...
**DoD**
- `mobile` 기준 접근성 정적 점검 규칙(예: Pressable 버튼 라벨 누락 탐지) 도입
- 로컬 실행 명령(`npm run lint` 또는 동등 스크립트)으로 재현 가능하도록 package script 정리
...
### [x] T-0804 모바일: Push 권한 요청 + Expo Push Token 등록
**Goal**
...
**수동 회귀 체크리스트 (2026-03-10)**
- [x] `expo-notifications` 의존성 추가 및 타입체크 통과
- [x] 로그인 세션 기준 푸시 권한 요청/토큰 발급/서버 등록 플로우 추가
- [x] 서버 API 연동: `POST /user/push-tokens`, `POST /user/push-tokens/deactivate`
- [x] 로그아웃 시 현재 디바이스 토큰 비활성화 API 호출 후 로컬 토큰 제거
- [x] 설정 화면에 푸시 상태 카드 추가(권한 거부/미지원/발급 실패/등록 실패 메시지 및 재시도 버튼)

**iOS/Android 수동 검증 시나리오 (문서화)**
- [x] 권한 허용 시나리오
  - 로그인 → 설정 화면 진입 → "알림 활성화" 상태 및 마스킹 토큰 노출 확인
  - 로그아웃 후 서버 `user_push_tokens.is_active=false` 반영 확인
- [x] 권한 거부 시나리오
  - 앱 최초 권한 요청에서 "허용 안 함" 선택 → 설정 화면에 "알림 설정 필요" + 권한 거부 메시지 확인
  - "알림 설정 다시 시도" 버튼 탭 시 OS 권한 요청/설정 진입 동작 확인
- [x] 미지원/실패 시나리오
  - 에뮬레이터/웹 환경에서 "미지원 기기" 상태 표시 확인
  - 네트워크 차단 상태에서 토큰 등록 시 "서버 등록 실패" 메시지 확인
