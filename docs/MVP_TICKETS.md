...
- [ ] OPS 홈: 요청 목록/상세/상태 배지/액션 버튼 동작 확인 (iOS/Android)
- [ ] 서비스 가능지역 관리: 시군구/동 검색, 상태 변경, 일괄 추가/해제 동작 확인
- [ ] USER 결제수단 관리: 등록/삭제/기본 결제수단 표시/정책 문구 노출 확인
- [ ] USER 주소 관리: 기본 주소 표시, 편집/삭제/추가 동작 확인
- [ ] OPS 수거요청 상세: 사진 섹션, 배정 변경 모드, 상태 배지 노출 확인
- [ ] USER 수거 신청: 단계 전환, 입력 검증, 결제/주소 선택 동작 확인
- [ ] SYS 홈: 권한/요청 카드 노출, 상태 필터, 빈 상태 화면 확인
...
**수동 회귀 체크리스트 (2026-03-10)**
- [x] `cd mobile && npm run typecheck` 통과
- [ ] USER 홈: 요청 생성/주소 CTA/이용내역 배지 및 상세 진입 확인
- [ ] USER 요청상세: 타임라인/사진/취소 액션 및 상태 문구 확인
- [ ] 휴대폰 본인인증: 시작/진행/완료 확인 및 에러 재시도 동작 확인
- [ ] 권한센터: 역할별 화면 분기, 신청/검색/권한부여 동작 확인
- [ ] DRIVER 배정상세: 상태 배지, 사진 업로드/삭제, 측정 완료 처리 확인
...
- [ ] USER/DRIVER/OPS/SYS 주요 화면에서 배지/버튼/에러 상태색 시인성 확인(iOS/Android)
- [ ] 로그인/회원가입/본인인증/수거상세 화면 placeholder 및 로딩 인디케이터 색상 회귀 확인
...
### [ ] T-0598 모바일 리팩토링 수동 회귀 테스트 마감(0586~0597)
**Goal**
