...
import { CreateWasteRequestPayload, WasteRequest, WasteRequestDetail } from '../types/waste';
...
export async function getMyWasteRequestDetail(requestId: number): Promise<WasteRequestDetail> {
  const response = await httpClient.get<WasteRequestDetail>(`/waste-requests/${requestId}`);
  return response.data;
}
